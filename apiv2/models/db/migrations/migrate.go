...
type MigrationRecord struct {
	...
	MigrationId types.MynahUuid `xorm:"INDEX UNIQUE"`
}

var migrations = make(map[types.MynahUuid]Migration)
...
func Migrate(engine *xorm.Engine) error {
	...
	defer func(s *xorm.Session) {
		if err := s.Close(); err != nil {
			log.Error("Failed to close db session after migration transaction: %s", err)
		}
	}(sess)

	rollback := func(s *xorm.Session) {
		if err := s.Rollback(); err != nil {
			log.Error("failed to rollback migration transaction")
		}
	}
	...
	for _, migration := range sortedMigrations {
		...
		if applied, err := migrationApplied(sess, migration); err != nil {
			...
			if err := applyMigration(sess, migration); err != nil {
				rollback(sess)
				log.Info("failed to apply migration[%s]: %s", migration.Id(), err)
				return err
			}

			log.Info("applied migration[%s]", migration.Id())
		}
	...
	log.Info("database has %d migrations applied", len(sortedMigrations))
	return nil
}
